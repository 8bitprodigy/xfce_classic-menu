...
classic_menu_construct(XfcePanelPlugin *plugin)
{
    ...
    GtkWidget *applications_menu, *places_menu, *system_menu;
    ...
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(menu->applications_item), applications_menu);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu->menubar), menu->applications_item);

    /* Create Places menu item */
    menu->places_item = gtk_menu_item_new_with_label("Places");
    places_menu = build_places_menu();
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(menu->places_item), places_menu);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu->menubar), menu->places_item);

    /* Create System menu item */
    menu->system_item = gtk_menu_item_new_with_label("System");
    system_menu = build_system_menu();
...
/* Handle size changes */
static gboolean
classic_menu_size_changed(XfcePanelPlugin *plugin, gint size, ClassicMenuPlugin *menu)
{
...
static void
classic_menu_orientation_changed(XfcePanelPlugin *plugin, GtkOrientation orientation, ClassicMenuPlugin *menu)
{
